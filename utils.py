...
def aggregate_dataset(df, grouping_field_list,  array_field):
    ...
    #        array_field].apply(lambda x: x[array_field].values.tolist()).reset_index().rename(columns={0: array_field + #"_array"})
    ...
    df = df.groupby(grouping_field_list)[non_grouped_field_list].agg(lambda x: list([y for y in x if y is not np.nan ] ) ).reset_index()

    dummy_df = pd.get_dummies(df[array_field].apply(pd.Series).stack()).sum(level=0)
